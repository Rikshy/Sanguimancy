...
public class RenderAltarManipulator extends TileEntitySpecialRenderer<TileAltarManipulator> {
    ...
    @Override
    public void render(TileAltarManipulator te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        renderModel(te, x, y, z);
        GL11.glPushMatrix();
        if (!te.getInventory(null).getStackInSlot(1).isEmpty()) {
            ItemStack displayItem = te.getInventory(null).getStackInSlot(1);
